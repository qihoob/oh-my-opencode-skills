...
# 替换占位符为你的实际路径（替换两次：先当前会话，再全局 CLAUDE.md）
...
将以下内容添加到你的 `~/.claude/CLAUDE.md`（**把 `$SKILL_DIR` 替换成你上一步的实际路径**）：
...
| 用户说 | 读取 |
|--------|------|
| 需求分析 | `$SKILL_DIR/product/requirement/product-requirement-analysis/SKILL.md` |
| 先看看代码 | `$SKILL_DIR/dev/context/dev-context-first/SKILL.md` |
| 实现功能 | `$SKILL_DIR/dev/implementation/dev-implementation/SKILL.md` |
| 代码审查 | `$SKILL_DIR/dev/code-review/SKILL.md` |
...
| 调试 | `$SKILL_DIR/dev/debugging/SKILL.md` |
| 重构 | `$SKILL_DIR/dev/refactoring/SKILL.md` |
